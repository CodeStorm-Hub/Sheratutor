...
| --- | --- | --- | --- |
| Standard pay-per-token | [Foundry Models sold by Azure](foundry-models__concepts__models-sold-directly-by-azure) | Per input and output token | Lowest-friction path to get started; bursty traffic on hosted models with no capacity planning. |
