---
title: System Architecture & AI
description: Full-stack system architecture, Google Genkit flow orchestration, Azure Foundry blueprints, codebase anatomy, and RAG design.
---

# System Architecture & AI Engineering

<div className="sheratutor-hero-badge">
  <span className="badge-tag">FULL-STACK CLOUD ARCHITECTURE</span>
  <span className="badge-status">NEXT.JS 16 &amp; GENKIT AI</span>
</div>

Welcome to the **SheraTutor Architecture & AI Engineering Hub**. This section documents the end-to-end technical implementation of SheraTutor, spanning the Next.js 16 App Router frontend, Supabase PostgreSQL 17 database with pgvector, Google Genkit AI orchestration flows, and Microsoft Azure AI Foundry deployment blueprints.

---

## Architecture & Engineering Directory

<div className="grid cards">

-   🏛️ **[System Architecture Report](system-architecture)**

    ---

    Complete architecture topology covering Next.js 16, React 19, Supabase SSR, pgvector indexing, and NVIDIA NIM vision extraction.

    [→ Inspect System Topology](system-architecture)

-   ⚙️ **[Codebase Anatomy Report](codebase-anatomy)**

    ---

    In-depth structural breakdown of `web/` and backend packages, API routes, Server Actions, state management, and shared components.

    [→ Explore Codebase Anatomy](codebase-anatomy)

-   ⚡ **[Google Genkit Architecture](genkit-architecture)**

    ---

    Detailed specification of Google Genkit v1.41+ flow engine, Reflection API, OpenTelemetry tracing, and plugin architecture.

    [→ Read Genkit Deep Dive](genkit-architecture)

-   🔄 **[SheraTutor + Genkit Integration](genkit-integration-plan)**

    ---

    Step-by-step implementation of 7 production AI flows (rubric evaluation, Bengali/English tutoring, LaTeX rendering, vector search).

    [→ View Integration Specs](genkit-integration-plan)

-   ☁️ **[Azure Foundry Blueprint](foundry-integration-blueprint)**

    ---

    Enterprise migration blueprint to Azure AI Foundry Agent Service, Agent Canvas, Model Router latency switching, and MCP server tooling.

    [→ Review Foundry Blueprint](foundry-integration-blueprint)

-   🗄️ **[Multimodal Vector DB Blueprint](multimodal-vector-foundry)**

    ---

    Architecture for scaling the NCTB multimodal vector database with Azure AI Search, hybrid reciprocal rank fusion, and Supabase pgvector.

    [→ Inspect Vector Blueprint](multimodal-vector-foundry)

-   👥 **[Developer Hand-Off Guide](developer-handoff-guide)**

    ---

    Complete engineering hand-off guide for B2C SSC Web Portal developers covering local setup, environment variables, and deployment workflows.

    [→ Open Hand-Off Guide](developer-handoff-guide)

-   📘 **[LLM, RAG & Vector Guide](llm-rag-vector-guide)**

    ---

    Practical guide to state-of-the-art LLMs, hybrid RAG retrieval pipelines, chunking heuristics, and AI orchestration best practices.

    [→ Read Technical Guide](llm-rag-vector-guide)

</div>

---

## Core System Technology Stack

```mermaid
flowchart TD
    subgraph Client["Presentation Layer (Client)"]
        UI["Next.js 16 App Router + React 19"]
        TW["Tailwind CSS v4 + Radix UI"]
    end

    subgraph Edge["API & Edge Compute"]
        SA["Next.js Server Actions & API Routes"]
        SSR["Supabase SSR Auth Session Middleware"]
    end

    subgraph AI["AI Orchestration Engine"]
        GK["Google Genkit v1.41+"]
        NIM["NVIDIA NIM Vision LLM"]
        BGE["BGE-M3 Multilingual Embeddings"]
    end

    subgraph Storage["Data & Vector Persistence"]
        PG["Supabase PostgreSQL 17"]
        VEC["pgvector (HNSW Indexing)"]
        BLOB["Supabase Storage (Exam Images)"]
    end

    UI --> SA
    SA --> SSR
    SA --> GK
    GK --> NIM
    GK --> BGE
    GK --> PG
    PG --> VEC
```
