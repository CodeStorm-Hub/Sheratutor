...
- **Student Profile Creation & Onboarding Flow**: Built in `web/src/app/onboarding/page.tsx` and `web/src/app/actions/onboarding.ts`.
  - Collects Date of Birth (calculates `is_minor` &le; 18 cutoff), Education Board (all 11 BD boards), Academic Group (`SCIENCE`, `HUMANITIES`, `BUSINESS_STUDIES`), and Target Year.
